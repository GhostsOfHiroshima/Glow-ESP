...
int ProcMem::iSizeOfArray( int* iArray ) {
	...
	//AddConsole("[ОШИБКА] Не удалось прочесть длину массива");
	return 0;
}
...
bool ProcMem::Process( char* ProcessName ) {
...
void ProcMem::Inject( DWORD dwAddress, char* Inj_Bts, char* Def_Bts, BOOL Type ) {
	...
	if ( !bIOn )
	{
		...
		//else { AddConsole("[ОШИБКА] Количество байтов по умолчанию должно быть > 5\n"); return; }
...
DWORD ProcMem::AOB_Scan( DWORD dwAddress, DWORD dwEnd, char* Bytes ) {
	...
	//AddConsole("[ОШИБКА] Не удалось найти шаблон байтов\n");
	return 0;
}

/* Returns The Base Address Of The Specified Module Inside The Target Process
/* e.g.[ Module("client.dll"); ]. */
bool ProcMem::Module( LPSTR ModuleName, DWORD& output ) {
	//Variables
	...
	//AddConsole("[ОШИБКА] Не удалось получить данные модуля \""+(string)ModuleName+"\"");
	return false;
}
